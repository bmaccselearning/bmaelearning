...
$department_name_new=$_REQUEST["department_name"];
$mode=$_REQUEST["mode"];
$returnurl = new moodle_url('/master/EditDept.php');
...
// echo "</br>department_name:".$department_name_new;
 ...
 if($mode=="save")
 {
  ...
  // echo "update department_name_new";
   $sql="update view_users_department set department_name='".$department_name_new."' where department_code='".$department_code."'";
   //echo "</br>sql:".$sql;
   $rs= $DB->execute($sql);
   if(!$rs){
    //echo "</br>error";
 ...
 $sql = "SELECT * from view_users_department where department_code='".$department_code."' ";
// echo "</br>sql:".$sql;
...
echo "<form name='formdep' id='formdep' method='GET' action=''>";
...
echo "<div class='form-group row fitem'><div class='col-md-3 col-form-label d-flex pb-0 pr-md-0'>ชื่อหน่วยงาน </div><div><input class='form-control' type='text'size='100'  name='department_name' value='".$departmentFt->department_name."' /></div></div>";
}
